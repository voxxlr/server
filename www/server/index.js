...
module.exports = (ACCOUNT, META, KEY, LINK, DATA) =>
{
    ...
    router.get(['/', '/index.html'], async (req, res, next) =>
    {
        ...
        let INDEX = mustache.render(fs.readFileSync(path.join(__dirname, 'index.html'), 'utf8'), params);
    ...
    router.get('/main.html', asyncHandler(async (req, res, next) =>
    {
        ...
        if (account)
        {
            ...
            let MAIN = mustache.render(fs.readFileSync(path.join(__dirname, 'main.html'), 'utf8'), params);
            res.type('html')
            res.send(MAIN);
        }
        else Auth.account_error(res, Http.UNAUTHORIZED);;
    ...
    router.get('/login.html', asyncHandler(async (req, res, next) =>
    {
        ...
        res.type('html')
        res.send(mustache.render(fs.readFileSync(path.join(__dirname, 'login.html'), 'utf8'), params));
    }))
